...
public class PaymentServiceImpl implements PaymentService {
    public Payment payPremium(String cid, String pid,
        ...
        try (SqlSession s = MyBatisUtil.openSession()) {
            ...
            if (pol == null)
                throw new PaymentProcessingException("Policy not found.");

            if (!cid.equals(pol.getCustomerId()))
                throw new AuthorizationException("Policy does not belong to customer.");

            if (pol.getPremiumStatus() == PremiumStatus.PAID)
                throw new PaymentProcessingException("Premium already paid.");

            if (Math.abs(amount-pol.getPremium())>.01)
                throw new PaymentValidationException("Amount must equal premium.");
            p.setId("PAY-" + LocalTime.now().
